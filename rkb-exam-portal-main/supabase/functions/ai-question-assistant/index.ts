...
async function callGeminiJson(
  ...
  const raw = await callGeminiRaw(apiKey, systemInstruction, userText, true);
  try {
...
async function runExamQualityAudit(
  ...
  const list = (rows || []) as ExamQuestionRow[];
...
For each MCQ: check whether correct_option (A-D) matches the option text that is actually correct for the stem. Flag if the true answer is missing from the four options, or if the wrong letter is marked.
For NUMERICAL/FILL types: check that correct_answer is present and plausible; options may be empty.
Return ONLY JSON: {"items":[{"id":"uuid","findings":["short bullet",...]}]}. Use findings: [] when there are no problems beyond heuristic_issues. Max 4 findings per question.`;

      try {
        const parsed = (await callGeminiJson(geminiKey, system, JSON.stringify(payload))) as {
        ...
        for (const it of parsed.items || []) {
          ...
          if (!id) continue;
          const findings = Array.isArray(it.findings)
            ? it.findings.map((x) => String(x)).filter(Boolean)
            : [];
        ...
        for (const item of items) {
          ...
          for (const e of extra) {
            if (!e.trim()) continue;
            const tagged = e.startsWith('[AI]') ? e : `[AI review] ${e}`;
            if (!item.issues.includes(tagged)) item.issues.push(tagged);
          }
          if (extra.length > 0 && item.severity === 'ok') {
            item.severity = 'warning';
          }
        }
      } catch (e) {
        console.warn('[Assistant] Gemini exam audit chunk failed:', e);
      }
    }
  }
  ...
  if (questions_with_issues === 0) {
    lines.push('No structural issues detected. Optionally re-run with GEMINI_API_KEY for deeper answer verification.');
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'audit_exam') {
      ...
      const audit = await runExamQualityAudit(supabase, exam_id, geminiKey);
      const BUILD_MARKER = 'ai-question-assistant@2026-05-02.audit';
      return new Response(
        JSON.stringify({
...
${lastUser}${ocrSnippetForForce}`;
    ...
    if (
      ...
      try {
        exportQuestions = (await applyGeminiRefinementsToQuestions(
          exportQuestions as Record<string, unknown>[],
          geminiKey
        )) as unknown[];
        geminiPostUsed = true;
        console.log('[Assistant] Gemini cross-check applied to generated questions.');
      } catch (e) {
        console.warn('[Assistant] Gemini cross-check skipped:', e);
      }
    }

    const BUILD_MARKER = 'ai-question-assistant@2026-05-02.gen';
    return new Response(
      JSON.stringify({
